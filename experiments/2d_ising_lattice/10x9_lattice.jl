...
function contract_3_loops(r1, r2, r3, tn, i, vs_centre)
    ...
    t3 = itensor(
        array(ITensorCPD.had_contract(outer.data_graph.vertex_data.values, r3)) .*
        array(cpopt_s3[]),
        r3,
    )
...
for rank = 1:length(ranks)
    ...
    r3 = Index(1, "CP_rank3")
    for beta in [1.0 - i for i = 0:0.01:1]
        for i = 1:2
            if i == 1
            ...
            val = contract_3_loops(r1, r2, r3, tn, i, vs_centre)
