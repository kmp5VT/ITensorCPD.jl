...
function contract_loop(r1, tn)
    ...
    val = (
        itensor(
            ...
            array(cpopt[]),
            r1,
        ) * ITensorCPD.had_contract([inner...], r1)
    )
    return val
end
...
for rank = 1:length(ranks)
    ...
    vals[1] = (contract([
        ...
        ψ[szverts...],
        dag(prime(ψ[szverts...])),
        ITensors.op("I", s[szverts...]),
    ]))[]
    vals[2] = (contract([
        cont,
        ψ[szverts...],
        dag(prime(ψ[szverts...])),
        ITensors.op(op_str, s[szverts...]),
    ]))[]
    cp = vals[2] / vals[1]
...
bp_numer = contract(
    [env_bp ψ[szverts...] dag(prime(ψ[szverts...])) ITensors.op(op_str, s[szverts...])];
    sequence = "automatic",
)[]
bp_denom = contract(
    [env_bp; ψ[szverts...]; dag(prime(ψ[szverts...])); ITensors.op("I", s[szverts...])];
    sequence = "automatic",
)[]
...
exact_denom = contract(
    [env_exact ψ[szverts...] dag(prime(ψ[szverts...])) ITensors.op("I", s[szverts...])];
    sequence = "automatic",
)[]
