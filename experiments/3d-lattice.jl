...
ψψ = ITensorNetwork([
    v => ψ[v] * noprime(prime(dag(ψ[v])); tags = "Site") for v in vertices(ψ)
]);
...
cp_flat_layer_2_1 =
    cp_flat_layer_2_1 * ITensorCPD.had_contract(cp_layer_2[l÷3+1:l÷3+l÷3], r2)
...
bp_numer = contract(
    [env_bp; ψ[v_centre]; dag(prime(ψ[v_centre])); ITensors.op(op_str, s[v_centre])];
    sequence = "automatic",
)[]
bp_denom = contract(
    [env_bp; ψ[v_centre]; dag(prime(ψ[v_centre])); ITensors.op("I", s[v_centre])];
    sequence = "automatic",
)[]
exact_numer = contract(
    [env_exact; ψ[v_centre]; dag(prime(ψ[v_centre])); ITensors.op(op_str, s[v_centre])];
    sequence = "automatic",
)[]
exact_denom = contract(
    [env_exact; ψ[v_centre]; dag(prime(ψ[v_centre])); ITensors.op("I", s[v_centre])];
    sequence = "automatic",
)[]
